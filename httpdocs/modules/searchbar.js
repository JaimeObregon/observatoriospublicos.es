...
export function initSearchBar(parent, observatories) {
  ...
  searchInput.addEventListener('input', (event) => {
    ...
    const filteredObservatories = filterObservatories(searchTerm, originalObservatories);
...
function addFilterElement(thisObservatories) {
  return thisObservatories.map(element => {
    const filterElement = filterKeys.map(attribute => normalizeText(element[attribute] || '')).join(' ');
...
function filterObservatories(searchTerm, thisOriginalObservatories) {
  ...
  return searchTerms.reduce((filteredObservatories, term) => {
    return filteredObservatories.filter(observatory => observatory.filterElement.includes(term));
...
function normalizeText(text) {
  return text.trim().normalize(normaliceStategy).replace(/[\u0300-\u036f]/g, "").toLowerCase();
}
